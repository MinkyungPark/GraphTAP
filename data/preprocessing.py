...
from examples.graph_diffusion.utils.utils import to_np, to_torch
...
class QuantileDiscretizer:
	...
	def _test(self):
		print('[ discretization ] Testing...', end=' ', flush=True)
	...
	def reconstruct(self, indices, subslice=(None, None)):
		...
		if indices.min() < 0 or indices.max() >= self.N:
			print(f'[ discretization ] indices out of range: ({indices.min()}, {indices.max()}) | N: {self.N}')
			indices = np.clip(indices, 0, self.N - 1)
...
# Kitchen, Ant Env 일 때 trajectory/datasets/preprocessing.py