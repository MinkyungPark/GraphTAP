...
from examples.graph_diffusion.modules import init_graphormer_params, GraphormerGraphEncoder
from examples.graph_diffusion.models.vqvae import VQTransformerDecoder

logger = logging.getLogger(__name__)


@register_model("graphormer_tap")
class GraphormerTapModel(FairseqEncoderDecoderModel):
    ...
    def build_model(cls, args, task):
        ...
        task.mcfg.enc_embed_dim = args.encoder_embed_dim
        task.mcfg.num_node = task.num_node
        encoder = GraphormerEncoder(args)
        decoder = VQTransformerDecoder(task.mcfg)
        return cls(args, encoder, decoder)

    def forward(self, batched_data, **kwargs):
        init_state = batched_data['seq_X'][:, 0, :self.decoder.observation_dim]
        
        graph_features = self.encoder(batched_data, **kwargs) # B*T, Node+1, dim
        
        B = batched_data['batch_size']
        T = batched_data['timesteps']
        graph_features = graph_features.view(B, T, -1).to(dtype=torch.float32) # flatten node features, 64, 10, 800
        
        joined_pred, latents, features = self.decoder(graph_features, init_state)
        return joined_pred, latents, features
...
class GraphormerEncoder(FairseqEncoder):
    ...
    def forward(self, batched_data, perturb=None, masked_tokens=None, **unused):
        ...
        x = inner_states[-1].transpose(0, 1) # N+1, B*T, D
...
@register_model_architecture("graphormer_tap", "graphormer_tap")
...
@register_model_architecture("graphormer_tap", "graphormer_tap_base")
...
@register_model_architecture("graphormer_tap", "graphormer_tap_slim")
...
@register_model_architecture("graphormer_tap", "graphormer_tap_large")
